...
def setGeneralContext(request, msg='Gene ID not found!'):
    ...
    obj = TechRecordsTable_Biorepository(
        NwAttributes11Biorepository.objects.filter(species='Just need to return a table'))
...
def innerQuery(pk):
    ...
    inner_queryset = NwCategoryLowestNodes.objects.filter(
        level2=level2).values('lowest_node')
...
def TableBindings(request, pk, cat):
    ...
    try:
        ...
        queryset_geneID = Table_looker[cat][0].objects.select_related('product_code').exclude(gene_id='').filter(
            product_code__category_1__in=inner_queryset, product_code__delete_flag=0, gene_id=geneID)
        obj = Table_looker[cat][1](queryset_geneID)
        gene_info = NcbiGeneInfo.objects.get(gene_id=geneID)
        filter_used = {'Gene ID': geneID, 'Gene Symbol': gene_info.gene_symbol,
                       'Gene Description': (gene_info.gene_description)}
        context = {'obj': obj, 'num_of_prods': queryset_geneID.count(),
                   'filter': filter_used}
        messages.success(
            request, 'Showing Products similar to Product code: ' + pk + ' in ' + cat + '>>' + lev)
        return render(request, "similarProducts.html", context)
    except NcbiGeneInfo.DoesNotExist:
        geneID = Table_looker[cat][0].objects.get(product_code=pk).gene_id
        if geneID == '' or geneID == None:
            geneID = "N\\A"
            queryset_cat = Table_looker[cat][0].objects.filter(
                product_code__category_1__in=inner_queryset, product_code__delete_flag=0, gene_id=geneID).exclude(gene_id='')[:1000]
...
def categoryWiseProductSorting(cat, pk, request):
    ...
    if cat == "Biorepository":
        inner_queryset, level2 = innerQuery(pk)
        queryset_base = NwAttributes11Biorepository.objects.get(
            product_code=pk)
        ...
        context = {'obj': obj, 'num_of_prods': queryset.count(), 'filter': {
            'Species': species, 'Tissue type': tissue_type, 'Disease': disease, }}
        messages.success(
            request, 'Showing Products similar to Product code: ' + pk + ' in ' + cat + '>>' + lev)
        return render(request, "similarProducts.html", context)
        ...
        if level2 in [129, 132]:
            queryset_base = NwAttributes15Cellscellculture.objects.get(
                product_code=pk)
            if level2 == 129:
        ...
        if level2 in [137, 138]:
            queryset_base = NwAttributes16Reagentslabware.objects.get(
                product_code=pk)
            if level2 == 137:
                ...
                filter_used = {
                    'Carbohydrate Type': queryset_base.carbohydrate_type}
        ...
        inner_queryset, level2 = innerQuery(pk)
        if level2 == 125:
            queryset_base = NwAttributes14Proteinspeptides.objects.get(
                product_code=pk)
            filter_used = {'Cell line': queryset_base.cell_line}
            ...
            context = {'obj': obj, 'num_of_prods': queryset.count(),
                       'filter': filter_used}
            messages.success(request, 'Showing Products similar to Product code: ' +
                             pk + ' in ' + cat + '>>Cell Line Lysates')
            return render(request, "similarProducts.html", context)
        else:
            return TableBindings(request, pk, cat)
    else:
